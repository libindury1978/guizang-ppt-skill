...
**若用户已提供大纲或素材**，仍须完成下方「初稿确认流程」（大纲 → 完整文案 → 每页初稿）并经用户确认后再进入 Step 2。**若只有主题或模糊想法**，先用 11 项必做清单逐项对齐。不要基于猜测就开始写 slide——一旦结构定错,后期翻修代价很高。

#### 必做清单（共 11 项，动手前必须全部确认）

| # | 问题 | 作用 |
|---|------|------|
| 1 | 从 0 开始还是有现成文档？ | 澄清起点 |
| 2 | 每一页的主题是否已确定？ | 确认结构 |
| 3 | 受众是谁？分享场景？ | 决定语言风格和深度 |
| 4 | 分享时长？（约 15 分钟 ≈ 10 页） | 决定总页数 |
| 5 | 有没有原始素材？ | 有则基于素材，无则帮你搭 |
| 6 | 有没有图片？ | 见下方图片约定 |
| 7 | 想要哪套主题色？ | 决定视觉基调（只能从 5 套里选） |
| 8 | 有没有硬约束？ | 避免返工 |
...
叙事弧 + 页数规划 + 主题节奏表(见 `layouts.md`),**三张表对齐且用户确认大纲后**,再推进至「初稿确认流程」；**须在用户确认全文案之后**,才开始在 `index.html` 中批量填充 slide 内容（Step 3）。

大纲建议保存为 `项目记录.md` 或 `大纲-v1.md`,便于后续迭代。

#### 页面层级与字数规范（必须提前告知用户）

每页信息分 **3 层**，超出层级的信息必须拆分到多页：

| 层级 | 内容 | 字数上限 |
|------|------|---------|
| **L1 · 标题层** | 页面主标题，用衬线字体 | ≤ 20 字 |
| **L2 · 核心层** | 关键论点 / 关键词 / 数据点 | ≤ 60 字 |
| **L3 · 辅助层** | 说明文字 / 注释 / 出处（可选） | ≤ 40 字 |

**字数计算**：中文字符计 1，标点计 0.5，空格不计入。英文单词计 1。

**为什么要规定层级**：电子杂志风格靠大字号和留白吃饭，一页塞太多字 = 没人看。每页只专注讲清楚 **1 个核心观点**，多了就拆成 2 页。

**给用户的建议**：
> "这一页你主要想让我记住什么？"——如果答案超过 1 个，就拆成 2 页。
...
项目名/ppt/images/
即：和 `index.html` 放在同一目录下。例如项目叫"品牌部AI工作"：

~/Desktop/品牌部AI工作/ppt/images/
...
示例：
- `01-cover.jpg` —— 第1页封面
- `02-section.jpg` —— 第2页章节幕封
- `03-pipeline.png` —— 第3页流程图
- `04-team.jpg` —— 第4页团队照
...
| 用途 | 格式 | 要求 |
|------|------|------|
| 照片 / 人物 / 场景 | JPG | ≥ 1600px 宽 |
| 截图 / UI 界面 / 透明背景图 | PNG | ≥ 1600px 宽 |
| 单张大小 | — | ≤ 5MB |
| 整个 images 文件夹 | — | ≤ 10MB |

**操作流程**：
1. 告知用户在项目/ppt/images/ 路径下放图片
2. 图片到位后才引用；未到位时，相关页面用占位色块生成结构

保持**同名覆盖**最稳（HTML 里不用改路径）；若文件名变更，全局搜 `images/旧名` 改成新名。layout 4/5/10 等图文混排页无图时无法验证最终视觉效果，需与用户对齐预期。

#### 初稿确认流程（强制执行）


大纲确认 ──→ 完整文案确认 ──→ 每页初稿确认 ──→ 生成 HTML ──→ 用户验收
     ↑              ↑                    ↑
  用户确认       逐字过目（最重要）     用户确认

**文案确认是最重要的一步**：大纲确认后，把每一页的 L1 / L2 / L3 全部文字内容逐字写出，给用户过目。只出纯文字稿，不出任何代码。用户确认后，才进入 HTML 生成环节。
...
5. **L1 主标题须符合 Step 1「页面层级与字数规范」（≤20 字）；必要时用 `nowrap` 避免窄屏下 1 字 1 行**
6. **用 Lucide,不用 emoji**
